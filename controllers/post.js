...
// Create a Socket.IO server attached to your Express server
const io = new Server(server);

// Add this function to emit progress
    ...
    const onVideoUploadSuccess = (videoUrl) => {
      jwt.verify(token, "secretkey", (err, userInfo) => {
        if (err) return res.status(403).json({status: false, message:"Token is not valid!"});
            ...
            const q =
            "INSERT INTO videoposts (`content`, `title`, `subtitle`, `playlist`, `videourl`, `thumbnail`, `userId`, `createdAt`) VALUES (?)";
          const values = [
            req.body.content,
            req.body.title,
            req.body.subtitle,
            req.body.playlist,
            videoUrl, // Use videoFile.originalname instead of req.body.videourl
            req.thumbnailFile,
            userInfo.id,
          ...
          db.query(q, [values], (err, data) => {
            if (err) return res.status(500).json({status: false, message: "There was an error during the uplod. Please try again later"});
 ...
 */
       ...
       /*  const onVideoUploadSuccess = (videoUrl) => {
        ...
        const video2 = {
          ...
          privacyStatus: "Public", // Set privacy status to private
          //publishType: 'PUBLIC',
          channelName: 'NaijaStreets',
          onSuccess:onVideoUploadSuccess,
          skipProcessingWait: true,
